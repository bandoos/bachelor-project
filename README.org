...
* Intro
  This document acts as technical appendix for my bachelor project.
...
* INPROGRESS Results analysis

  Once the desired experiment csv file has been retrieved
  form the database with the =dctl= utility, the analysis
  can be performed by the R script provided in =$PROJECT_ROOT/pos-sim-r/analyze.Rmd=.

  Compiling R dependencies may take a lot of time (nearly 15 minutes
  for the dependencies of =analyze.Rmd=, and errors in the process may
  harm the reproducibility of the analysis. Therefore a third
  docker image is provided which ships with all the dependencies
  compiled in it, and when run exposes an R-studio web interface
  to run (and possibly customize) the analysis.
  ...
  *Load the image to the docker engine*
  ...
  Enter =$PROJECT_ROOT/pos-sim-r= dir and run:

  ~$ docker-compose up~

  Point a browser to R-studio web-ui on localhost:8787.
  ...
  *Alternatively run from the rstudio terminal:* ~$ make render~
  ...
  TODO put link to analysis script and pdf.
  TODO use figure folder for png generation
